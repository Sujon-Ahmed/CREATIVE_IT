...
if (isset($_POST['submit'])) {
    ...
    if ($sub_title != '' && $title != '' && $description != '') {
        if (!empty($_FILES['file']['name'])) {
            ...
            if (in_array($actual_extension, $allowed_extension)) {
                ...
                if ($file_size <= 1000000) {
                    ...
                    if ($file_error == 0) {
                        ...
                        $update_data = "UPDATE `banners` SET `banner_sub_title`='$sub_title',`banner_title`='$title',`banner_description`='$description',`banner_image`='$file_new_name' WHERE id = $id";
            ...
            $update_data = "UPDATE `banners` SET `banner_sub_title`='$sub_title',`banner_title`='$title',`banner_description`='$description', `banner_image`='$old_image' WHERE id = $id";
            $update_data_result = mysqli_query($con, $update_data);
            $_SESSION['update_success'] = "Banner Update Successfully";
            header('location:view_banner.php');
        }
